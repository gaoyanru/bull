angular.module('channelApp').controller('ReviewCtrl', ['$scope', '$http', '$filter', '$uibModal','PyCapital', function($scope, $http, $filter, $uibModal,PyCapital) {
    'use strict';
    var tbOptions = [{
        header: '省',
        col: 'ProvinceName',
        ...
        header: '公司名称',
        col: 'Name',
        formate: function(val, row) {
            if (row.FreChangeOrderId) {
                return val + "<br/>(企业性质变更)";
            }
            return val;
        }
    }, {
    ...
    $scope.audit = function(order) {
        if (order.FreChangeOrderId) {
            $http.put('/api/signacontract/audit/' + order.OrderId).success(function(result) {
                if (result.status) alert('操作成功！');
                $scope.searchFn();
            });
        } else {
            $http.put('/api/orders/audit/' + order.OrderId).success(function(result) {
                if (result.status) alert('操作成功！');
                $scope.searchFn();
            });
        }
    ...
    $scope.view = function(orderId, ischange) {
        var modalInstance = $uibModal.open({
        ...
        data.forEach(function(row) {
            ...
            tbOptions.forEach(function(item) {
                if (item.col.indexOf(',') > 0) {
                    ...
                    var cv0 = (item.formate ? item.formate(row[c[0]], row) : row[c[0]]) || "无";
                    var cv1 = (item.formate ? item.formate(row[c[1]], row) : row[c[1]]) || "无";
                    cols.push(cv0 + '<br/>' + cv1);
                } else {
                    var cv = (item.formate ? item.formate(row[item.col], row) : row[item.col]) || "无";
                    cols.push(cv);
                }
            });
            row.cols = cols;
    ...
    function refreshData(data) {
        ...
        $http.get('/api/orders/agent?' + jQuery.param(data)).success(function(result) {
